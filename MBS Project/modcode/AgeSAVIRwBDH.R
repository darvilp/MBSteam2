...
pars <- c(
		contacty= 0.03,  	
		recovy 	= 0.02,   
		ldeathy = 0.004633 , #disease independent death
		hdeathy	= .0068, #disease induced death for young
		vacy	= .03,
		aqy		= .005,
		...
		contact	= 0.02,  	
		recov 	= 0.01,   
		ldeath  = 0.00567, #disease independent death
		hdeath	= .0068, #disease induced death for old
		vac		= .03,
		aq		= .01,
		birth	= .0243, #This can be lower if Civ vaccination is removed
		civ		= .00)	

init.values <- c(Sy = .35, Ay = .0, Vy = .005, Iy = .05, Ry = 0,
                 Cy = .00, Hy = 0, Ly = 0, S = .65, A = .0, V = .005,
                 I = .005, R = 0, C = .00, H = 0, L = 0)
times <- seq(0, 1000, by = 1)
SAVIR <- function(time, y.values, parameters) {
	with(as.list(c(y.values, parameters)), {
				dSy.dt = birth*(S+A+V+I+R)-aqy*Sy-(vac)*Sy-contacty*Iy*Sy-ldeathy*Sy-Cy/75*Sy-aging*Sy+dehyd*Vy
				...
				dS.dt =	-aq*S-(vac)*S-contact*I*S-ldeath*S-C/75*S+aging*Sy+dehyd*V

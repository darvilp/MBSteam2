...
pars <- c(
		...
		vac		= .07,
		aq		= .03,
		birth	= .1, #This can be lower if Civ vaccination is removed
		civ		= .0)	

init.values <- c(Sy = .3, Ay = .0, Vy = .005, Iy = .05, Ry = 0,
                 Cy = .0, Hy = 0, Ly = 0, S = .6, A = .0, V = .005,
...
matplot(out$time, out[c(16,17)], type = "l", xlab = "time", 
        ylab = "num", main = "Deaths of Adults", lwd = 2)

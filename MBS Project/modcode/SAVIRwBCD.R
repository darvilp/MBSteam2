...
pars <- c(	
		...
		vac		= .07,
		aq		= .05,
		birth	= .07, #This can be lower if Civ vaccination is removed
		civ		= .01)
init.values <- c(S =.5,A=.49,V=.005,I =.005, R = 0,C=.01,H=0,L=0)
times <- seq(0, 1000, by = 1)
SAVIR <- function(time, y.values, parameters) {
	with(as.list(c(y.values, parameters)), {
				
				dS.dt =	birth*(S+A+V+I+R)-aq*S-(vac)*S-contact*I*S-ldeath*S#-C/75*S
...
matplot(out$time, out[ ,2:6], type = "l", xlab = "time", 
		ylab = "Percent of Original Population", main = "SAVIR Model with Birth/Death", lwd = 2)

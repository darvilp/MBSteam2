...
pars <- c(
		...
		vac		= .07,
		aq		= .3,
		birth	= .3,#This can be lower if Civ vaccination is removed
		civ		=.00)	
init.values <- c(Sy =.5,Ay=.49,Vy=.005,Iy =.005, Ry = 0,Cy=.00,Hy=0,Ly=0, S =.5,A=.49,V=.005,I =.005, R = 0,C=.00,H=0,L=0)
...
SAVIR <- function(time, y.values, parameters) {
	with(as.list(c(y.values, parameters)), {
				...
				dS.dt =	birth*S-aq*S-(vac)*S-contact*I*S-ldeath*S-C/75*S+aging*Sy
